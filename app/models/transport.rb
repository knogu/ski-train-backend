class Transport < ApplicationRecord
  ...
  def json(prev_transport_id = nil)
    if prev_transport_id.present?
      transfer = Transfer.find_by(station_id: start_station_id, line_1_id: [Transport.find(prev_transport_id).line_id, line_id].min, line_2_id: [Transport.find(prev_transport_id).line_id, line_id].max)
